...
class TrainIter(mp.Process):
    def __init__(self, id, pow_max=None, num_ap=6, num_users=4, num_irs_x=4, 
        ...
        self.plot_zo_q2 = np.reshape(np.frombuffer(p_zo_q2, dtype=np.float64), (num_exp, num_iterations))
        self.plot_zo_q3 = np.reshape(np.frombuffer(p_zo_q3, dtype=np.float64), (num_exp, num_iterations))
        ...
        self.zo_q2 = ZerothOrder(self.env, pow_max, [num_irs_tot],sigma_2,
                        alphas, wmmse_iter, lr_th, mu, vec_theta=vec_theta, quant_bits=2)
        self.zo_q3 = ZerothOrder(self.env, pow_max, [num_irs_tot],sigma_2,
                        alphas, wmmse_iter, lr_th, mu, vec_theta=vec_theta, quant_bits=3)


        self.zo_q2.newtons_init_val = 1
        self.zo_q3.newtons_init_val = 1
    ...
    def run(self):
            ...
            for i in range(self.num_iterations):
                ...
                self.plot_zo_q2[self.id, i] = self.zo_q2.step(lr_decay_factor=decay_factor)
                self.plot_zo_q3[self.id, i] = self.zo_q3.step(lr_decay_factor=decay_factor)
                if (i+1)%self.log_iter==0: 
                    print("Exp ",self.id+1," i:",i+1,
                        " ZO Q=2:",np.average(self.plot_zo_q2[self.id,((i+1)-self.log_iter):i]),
                        " ZO Q=3:",np.average(self.plot_zo_q3[self.id,((i+1)-self.log_iter):i]))
...
if __name__ == "__main__":
    ...
    p_zo_q2 = mp.RawArray('d', num_exp*num_iterations)
    p_zo_q3 = mp.RawArray('d', num_exp*num_iterations)
...
#plot results
    plot_zo_q2 = np.reshape(np.frombuffer(p_zo_q2, dtype=np.float64), (num_exp, num_iterations))
    plot_zo_q3 = np.reshape(np.frombuffer(p_zo_q3, dtype=np.float64), (num_exp, num_iterations))
    # np.save('/home/radio/hassaan/dpgzo/scripts_mp/plot_data/main/v1/plot_zo_q2_v1.npy', plot_zo_q2)
    # np.save('/home/radio/hassaan/dpgzo/scripts_mp/plot_data/main/v1/plot_zo_q3_v1.npy', plot_zo_q3)

    plt.figure(figsize=(12, 8), dpi=80)
    plt.plot(movmean(np.average(plot_zo_q2, axis=0), 2000), color='g', label='ZO Q2')
    plt.plot(movmean(np.average(plot_zo_q3, axis=0), 2000), color='k', label='ZO Q3')

    
    #plotting annotations
    plt.xlabel("Iteration")
    plt.ylabel("Sumrate")
    plt.title("ZO Q2 vs ZO Q3")
    plt.legend()
    plt.show()
